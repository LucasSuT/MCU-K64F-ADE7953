...
static int network_status = NSAPI_STATUS_DISCONNECTED; // sophie, add API to get network status
...
int mcc_platform_get_network_status(void) {
	printf("mcc_platform_get_network_status: %d\n", network_status);
...
void network_status_callback(nsapi_event_t status, intptr_t param)
{
    ...
    if (status == NSAPI_EVENT_CONNECTION_STATUS_CHANGE) {
		    network_status = (int)param; // sophie, add API to get network status
		    printf("mcc network_status_callback: %d\n", network_status);
        switch(param) {
