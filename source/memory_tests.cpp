...
void print_stack_statistics()
{
    ...
    //<sophie, show more info
    mbed_stats_thread_t *thstats = (mbed_stats_thread_t*) malloc(cnt * sizeof(mbed_stats_thread_t));
    if (thstats) {
        cnt = mbed_stats_thread_get_each(thstats, cnt);
        for (int i = 0; i < cnt; i++) {
            printf("Thread(thread): 0x%(%s)" PRIx32 ", Stack size: %" PRIu32 ", Stack space: %" PRIu32 "\r\n", thstats[i].id, thstats[i].name, thstats[i].stack_size, thstats[i].stack_space);
        }
