...
//<sophie, add for callback when registered
...
public:
    ...
    SimpleM2MClient() :
        ...
        _on_registered(NULL){		// sophie, add for callback when registered
    }

    //<sophie, add for callback when registered
    void on_registered(_VoidVoidCb fp)
    {
        _on_registered = fp;
    }
    //>sophie, add for callback when registered
    ...
    void client_registered() {
        ...
        if (endpoint == NULL) {
            ...
            if (endpoint) {
                printf("Account ID: %s\r\n", endpoint->account_id.c_str()); // sophie add
        ...
        //<sophie, add for callback when registered
        if(_on_registered != NULL)
            _on_registered((void *)endpoint->endpoint_name.c_str());
        //>sophie, add for callback when registered
    }
    ...
    void error(int error_code) {
        ...
        switch(error_code) {
            ...
            case MbedCloudClient::UpdateErrorConnection:
				...
				OS_Reboot(DEFAULT_OS_REBOOT_WITH_MODEM_RESET_CONFIG); // sophie, reboot to make it reset and do full registration to make pelion to know update failed
                break;
#endif
    ...
    //<sophie, modify for both put and post
    ...
    M2MResource* add_cloud_resource(uint16_t object_id, uint16_t instance_id,
                              ...
                              M2MResourceInstance::ResourceType data_type,
                              M2MBase::Operation allowed, const char *value,
                              bool observable, void *put_cb, void *post_cb, void *message_status_cb) {
            return add_resource(&_obj_list, object_id, instance_id, resource_id, resource_type, data_type,
                      allowed, value, observable, put_cb, post_cb, message_status_cb);
    //>sophie, modify for both put and post
    }
...
private:
	...
	_VoidVoidCb           _on_registered;		// sophie, add for callback when registered
};
